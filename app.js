...
const toStream = async (parsed, uri, tor, type, s, e) => {
    ...
    if (!parsed.files && uri.startsWith("magnet")) {
        try {
            ...
            parsed.files = res;
            engine.destroy(); // Close the engine after use
        } catch (error) {
    ...
    if (type === "series") {
        index = (parsed.files || []).findIndex((element) => {
            const seasonFormatted = `s${s}`.padStart(3, "0");
            const episodeFormatted = `e${e}`.padStart(3, "0");
            const extArray = [".mkv", ".mp4", ".avi", ".flv"];

            return (
                element["name"]?.toLowerCase()?.includes(seasonFormatted) &&
                element["name"]?.toLowerCase()?.includes(episodeFormatted) &&
                extArray.some((ext) => element["name"]?.toLowerCase()?.includes(ext))
            );
        });
...
const fetchTorrentFromHost1 = async (query) => {
  ...
  const url = `${hostUrl}/api/v2.0/indexers/all/results?apikey=${apiKey}&Query=${query}&Category[]=2000&Category[]=2040&Category[]=2045&Category[]=2080&Category[]=5000&Category[]=5040&Category[]=5045&Category[]=5080&Category[]=100003&Category[]=100011&Category[]=100042&Category[]=100055&Category[]=100070&Category[]=100076&Tracker%5B%5D=bitsearch&Tracker%5B%5D=solidtorrents`;
...
const fetchTorrentFromHost2 = async (query) => {
  ...
  const url = `${hostUrl}/api/v2.0/indexers/all/results?apikey=${apiKey}&Query=${query}&Category[]=2000&Category[]=2040&Category[]=2045&Category[]=2080&Category[]=5000&Category[]=5040&Category[]=5045&Category[]=5080&Category[]=100011&Category[]=100003&Category[]=100042&Category[]=100055&Category[]=100070&Category[]=100076&Tracker[]=torrentgalaxy&Tracker%5B%5D=bitsearch&Tracker%5B%5D=bulltorrent`;
